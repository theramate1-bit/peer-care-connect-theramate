...
async function findExistingWebhook() {
  ...
  if (matching.length > 0) {
    console.log(`✅ Found ${matching.length} existing webhook(s) with matching URL:`);
...
async function main() {
  ...
  try {
    ...
    if (webhook.secret) {
      ...
      console.log('⚠️  Webhook secret not available. You may need to retrieve it from Stripe Dashboard.');
    }
    ...
    console.log(`   Secret: ${webhook.secret ? 'Set in Supabase' : '⚠️  Needs manual setup'}`);
    console.log('');
