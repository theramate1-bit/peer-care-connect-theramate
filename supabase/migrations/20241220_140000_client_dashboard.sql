-- Phase 4: Client Dashboard - Database Schema
-- This migration adds comprehensive client dashboard functionality
...
CREATE INDEX idx_projects_therapist_id ON public.projects(therapist_id);
...
CREATE INDEX idx_project_phases_status ON public.project_phases(phase_status);
...
CREATE INDEX idx_project_documents_status ON public.project_documents(document_status);
CREATE INDEX idx_project_messages_project_id ON public.project_messages(project_id);
CREATE INDEX idx_project_messages_sender ON public.project_messages(sender_id);
...
CREATE INDEX idx_project_payments_status ON public.project_payments(payment_status);
...
CREATE POLICY "Therapists can view client profiles for their projects" ON public.client_profiles
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM public.projects 
            WHERE projects.client_id = client_profiles.id 
